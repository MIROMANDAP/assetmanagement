...
function generateRandomNumber($length = 4) {
    return str_pad(random_int(0, 9999), $length, '0', STR_PAD_LEFT);
}

// Asset tag generation logic
if ($asset_type === "IT Peripherals") {
    // Generate a random asset tag for IT Peripherals
    $randomNumber = generateRandomNumber();
    $randomLetter = generateRandomLetter(1);
    $asset_number = "APAC-" . substr($date_acquired, 0, 4) . "-ITPE" . $randomNumber . $randomLetter . "";
} else {
    // Default asset tag generation for other asset types
        ...
        <div class="row">
            <div class="col">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off" enctype='multipart/form-data'>
                    ...
                    <label for="equipment_name">Equipment Description</label>
