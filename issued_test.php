...
$employees = [];
$sql = "SELECT employee_name FROM employee_list ORDER BY employee_name ASC";
if ($result = $mysqli->query($sql)) {
    while ($row = $result->fetch_assoc()) {
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($issued_to !== $previous_row['issued_to']) {
                $changes[] = "Change the assign asset from: " . htmlspecialchars($previous_row['issued_to']) . " to " . htmlspecialchars($issued_to);
            }
            ...
            echo "<script>alert('The Asset has been successfully Assign.'); window.location.href='edit_asset.php';</script>";
        } else {
...
<body>
    ...
    <div class="row">
        <div class="col-md-6">
		 ...
		 <div class="col-md-6">
            ...
            <input type="text" name="equipment_name" id="equipment_name" class="form-control" value="<?php echo htmlspecialchars($row['equipment_name']); ?>"readonly>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
			...
			<input type="text" name="status" id="status" class="form-control" value="Deployed" readonly>
    ...
    <div class="col-md-12">
        ...
        <select name="issued_to" id="issued_to" class="form-control">
            ...
            <?php foreach ($employees as $employee): ?>
                <option value="<?php echo htmlspecialchars($employee); ?>" <?php if ($employee === $row['issued_to']) echo 'selected'; ?>>
